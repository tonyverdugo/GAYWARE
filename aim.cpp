...
void legitbot::do_aimbot(C_BasePlayer *local, C_BaseCombatWeapon *weapon, CUserCmd *cmd)
{
	...
	if (!g_Options.aim_LegitBotRifles && weapon->IsRifle() | !g_Options.aim_LegitBotPistols && weapon->IsPistol())
		return;
...
void legitbot::weapon_settings(C_BaseCombatWeapon* weapon)
{
	...
	if (weapon->IsPistol())
	{
		...
		aim_smooth = g_Options.aim_PistolsSmooth;
		aim_fov = g_Options.aim_PistolsFOV;
		randomized_smooth = 1;
		recoil_min = g_Options.aim_PistolsRCSMin;
		recoil_max = g_Options.aim_PistolsRCSMax;
		randomized_angle = 1;
	}
