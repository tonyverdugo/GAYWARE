...
public:
	legitbot();
	void OnCreateMove(CUserCmd *pCmd, C_BaseEntity* local);
private:
	// Targetting
	...
	void do_aimbot(C_BaseEntity *local, C_BaseCombatWeapon *weapon, CUserCmd *cmd);
	...
	int get_target(C_BaseEntity *local, C_BaseCombatWeapon *weapon, CUserCmd *cmd, Vector &destination);
