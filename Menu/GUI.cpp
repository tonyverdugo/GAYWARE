...
void CWindow::Paint(int t_x, int t_y, float* controlheight)
{
	...
	Draw::FilledRectangle(tl.x, tl.y - 16, br.x, br.y, Color(83, 10, 196, 135));
	Draw::Rectangle(tl.x, tl.y - 16, br.x, br.y, Color(83, 10, 222));
	Draw::Rectangle(tl.x + 1, tl.y - 15, br.x - 1, br.y - 1, Color(83, 10, 185));
	tl += 6;
	br -= 6;
	Draw::FilledGradientRectangle(tl.x, tl.y, br.x, br.y, Color(238, 238, 238), Color(210, 210, 210));
	Draw::Rectangle(tl, br, Color(50, 5, 5));
	Draw::Rectangle(tl.x - 1, tl.y - 1, br.x + 1, br.y + 1, Color(83, 10, 222));

	Draw::Text(tl.x + 3, tl.y - 17, L"GAYWARE™ - A FAKEWARE™ paste", Font::Get().MenuText, Color::White);
